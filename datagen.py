...
def main():
    ...
    texts_dir = os.path.join(cfg.data_dir, cfg.texts_dir)
    ...
    with open(os.path.join(cfg.data_dir, "i_t.txt"), "w", encoding="utf-8") as i_t_f:
        ...
        for idx in range(cfg.sample_num):
            ...
            texts_path = os.path.join(texts_dir, str(idx).zfill(digit_num) + '.txt')
            ...
            i_t_f.write(f"{str(idx).zfill(digit_num)}.png {target_text}\n")  # i_t.txt
